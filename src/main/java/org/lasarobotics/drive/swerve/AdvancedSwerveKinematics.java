...
public class AdvancedSwerveKinematics {
  ...
  public static ChassisSpeeds correctForDynamics(ChassisSpeeds requestedSpeeds) {
    Pose2d futureRobotPose = new Pose2d(
      requestedSpeeds.vxMetersPerSecond * GlobalConstants.ROBOT_LOOP_FREQUENCY.asPeriod().in(Units.Seconds),
      requestedSpeeds.vyMetersPerSecond * GlobalConstants.ROBOT_LOOP_FREQUENCY.asPeriod().in(Units.Seconds),
      Rotation2d.fromRadians(requestedSpeeds.omegaRadiansPerSecond * GlobalConstants.ROBOT_LOOP_FREQUENCY.asPeriod().in(Units.Seconds))
    );

    Twist2d twistForPose = log(futureRobotPose);

    ChassisSpeeds correctedSpeeds = new ChassisSpeeds(
      twistForPose.dx / GlobalConstants.ROBOT_LOOP_FREQUENCY.asPeriod().in(Units.Seconds),
      twistForPose.dy / GlobalConstants.ROBOT_LOOP_FREQUENCY.asPeriod().in(Units.Seconds),
      twistForPose.dtheta / GlobalConstants.ROBOT_LOOP_FREQUENCY.asPeriod().in(Units.Seconds)
    );
  ...
  public static SwerveModuleState getRealModuleVelocity(ChassisSpeeds realSpeeds, Translation2d moduleLocation) {
    ...
    firstOrderInputMatrix.set(0, 0, realSpeeds.vxMetersPerSecond);
    firstOrderInputMatrix.set(1, 0, realSpeeds.vyMetersPerSecond);
    firstOrderInputMatrix.set(2, 0, realSpeeds.omegaRadiansPerSecond);
